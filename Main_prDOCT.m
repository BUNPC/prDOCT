%% Phase Resolved Doppler OCT for capillary axial blood flow velocity imaging, CPU-based
% input: 
...
disp(['Loading data to select the field of focus... ', num2str(floor(N_YChks/2)), ', ',datestr(now,'DD:HH:MM')]);
...
for iChk=1:N_YChks
    disp(['Start loading ith Chunk - ', num2str(iChk), ', ',datestr(now,'DD-HH:MM:SS')]);tic; 
    ...
    RR0 = DAT2RR(DAT, intDk);         % CPU-based, process raw spectrum data to spatial reflectivity data 
%     RR0 = DAT2RR_GPU(DAT, intDk);     % GPU-based, process raw spectrum data to spatial reflectivity data 
    ...
    Vz(:,:,(iChk-1)*nyPerChk+1:iChk*nyPerChk)=RR2Vz(RR, PRSinfo)*1e3; toc % CPU-based prDOCT processing, mm/s
%   Vz(:,:,(iChk-1)*nyPerChk+1:iChk*nyPerChk)=RR2Vz_GPU(RR, PRSinfo)*1e3; toc  % GPU-based prDOCT processing, ~25 times faster than CPU
end
